...
function build2ArchiveColumns(database: DatabaseSync) {
  return database.prepare("PRAGMA table_info(archive_events)").all() as unknown as {
    name: string;
  }[];
}

export function needsBuild2ArchiveMigration(database: DatabaseSync) {
  const columns = build2ArchiveColumns(database);
  return columns.length > 0 && columns.every(({ name }) => name !== "provider");
}

export function migrateBuild2Archive(database: DatabaseSync) {
  const columns = build2ArchiveColumns(database);
  assert.deepEqual(
